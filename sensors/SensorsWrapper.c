...
void init_sensors(void) {
    ...
    handle = dlopen(HAL_VENDOR_PATH"/sensors.nosensorhub.so", RTLD_NOW);

    if (!handle) {
        ALOGE("init_sensors: dlopen() failed to load \"sensors.nosensorhub.so\"");
        goto fail;
    }
    ...
    for (idx = 0; idx < sensors_count; ++idx) {
        ...
        // Proximity should always be a wake up sensor
        if (SENSOR_TYPE_PROXIMITY == global_sensors_list[global_sensors_count].type) {
               global_sensors_list[global_sensors_count].flags |= SENSOR_FLAG_WAKE_UP;
        }
